...
export function getViewportGridDimensions(count: number): {
  cols: number
  rows: number
} {
  if (count <= 0) {
...
function fractionsForLayout(raw: number[], targetLen: number): number[] {
  if (targetLen <= 0) {
    return []
  }
...
const TYPE_LABEL: Record<ViewportType, string> = {
...
function cumulativeDividers(fractions: number[], containerSize: number): number[] {
  const out: number[] = []
  let cum = 0
...
function nearPixel(a: number, b: number, tol = 2): boolean {
...
export function computeViewportResizeHandles(
  ...
  const handles: ViewportResizeHandleRect[] = []
  let hid = 0
